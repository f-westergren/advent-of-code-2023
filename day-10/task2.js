...
fs.readFile("input.txt", function read(err, data) {
  ...
  let steps = 0;
  let finish;
  let pipeMapCopy = JSON.parse(JSON.stringify(pipeMap));
  ...
  const walk = (dir) => {
    const {y, x} = position; 
    pipeMap[y][x] = "X";
    if (pipeMapCopy[y][x] === '|') pipeMapCopy[y][x] = '!';
    else if (pipeMapCopy[y][x] === 'L') pipeMapCopy[y][x] = '!';
    else if (pipeMapCopy[y][x] === 'J') pipeMapCopy[y][x] = '!';
    else if (pipeMapCopy[y][x] === 'S') pipeMapCopy[y][x] = '!';
    else pipeMapCopy[y][x] = '_';
    if (dir === "n") {
      position.y -= 1;
    }
    if (dir === "s") {
  ...
  pipeMapCopy = pipeMapCopy.map(row => row.filter(char => char !== '_'))

  let insideTotal = 0;
  pipeMapCopy.forEach((row) => {
    let inside = 0;

    row.forEach(char => {
      if (char === '!') inside +=1
      if (inside % 2 === 1 && char !== '!') insideTotal +=1
    })
  })

  console.log('Part 2:', insideTotal)
